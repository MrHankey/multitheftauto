...
int CLuaFunctionDefs::LoadSpecialCharacter ( lua_State * luaVM )
{
    int iArgument1 = lua_type ( luaVM, 1 );
    if ( ( iArgument1 == LUA_TNUMBER || iArgument1 == LUA_TSTRING ) &&
         lua_istype ( luaVM, 2, LUA_TSTRING ) )
    {
        ...
        const char * szSpecialCharacter = lua_tostring ( luaVM, 2 );

        if ( CStaticFunctionDefinitions::LoadSpecialCharacter ( usModel, szSpecialCharacter ) )
        {
            lua_pushboolean ( luaVM, true );
            return 1;
        }
        ...
        m_pScriptDebugging->LogBadType ( luaVM, "loadSpecialCharacter" );
...
int CLuaFunctionDefs::UnloadSpecialCharacter ( lua_State * luaVM )
{
    int iArgument1 = lua_type ( luaVM, 1 );
    if ( iArgument1 == LUA_TNUMBER || iArgument1 == LUA_TSTRING )
    {
        ...
        if ( CStaticFunctionDefinitions::UnloadSpecialCharacter ( usModel ) )
        {
            lua_pushboolean ( luaVM, true );
            return 1;
        }
        ...
        m_pScriptDebugging->LogBadType ( luaVM, "unloadSpecialCharacter" );
