...
void CFoo::Test ( const char* szString )
{
    ...
    else if ( strnicmp ( szString, "interp", 6 ) == 0 )
    {
        if ( pVehicleManager->Count () > 0 )
        {
            ...
            vecT.fZ = fdelta;
            pVehicle->SetTargetPosition ( vecT, 1000 );
    ...
    else if ( strnicmp ( szString, "interr", 6 ) == 0 )
    {
        if ( pVehicleManager->Count () > 0 )
        {
            ...
            pVehicle->SetTargetRotation ( vecT, 1000 );
    ...
    */
    ...
    else if ( strnicmp ( szString, "fuckveh", 7 ) == 0 )
    {
        ...
        if ( pVehicle )
        {
            pVehicle->SetTargetPosition ( CVector ( 0, 0, 0 ), 0 );
            pVehicle->SetTargetRotation ( CVector ( 0, 0, 0 ), 0 );
